...
public struct AssistantCLI: Identifiable, Hashable, Sendable {
    ...
    /// How to hold a conversation with this CLI, or `nil` when Loadout doesn't know how.
    ///
    /// `claude` and `codex` were each tried against a throwaway skill to find out. Everything else
    /// gets `nil` — a custom entry's flags are whatever the owner typed, and guessing that some
    /// unknown binary understands `--resume` would break it. Those keep the one-shot Ask.
    public var chat: AssistantChat? {
...
public enum AssistantCLIRegistry {
    ...
    /// The ones the Ask panel can hold a conversation with, for its own "none installed" tooltip.
    /// The other built-ins are still discovered and still counted — they just aren't asked.
    public static var chatCapableLabels: [String] {
        builtins.filter { AssistantChat.builtin(id: $0.id) != nil }.map(\.label)
    }
