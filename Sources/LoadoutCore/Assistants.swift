...
public struct Assistant: Identifiable, Hashable, Sendable {
    ...
    /// Where this assistant keeps its slash commands.
    ///
    /// Claude Code calls them commands and keeps them in `~/.claude/commands`; Codex calls them
    /// prompts and keeps them in `~/.codex/prompts`. Same thing — a markdown file named after the
    /// command — under two names, so the folder is chosen by whichever exists and falls back to the
    /// assistant's own convention when neither does yet.
    public var commandsRoot: URL {
        let home = skillsRoot.deletingLastPathComponent()
        let commands = home.appendingPathComponent("commands")
        let prompts = home.appendingPathComponent("prompts")
        let fm = FileManager.default
        if fm.fileExists(atPath: commands.path) { return commands }
        if fm.fileExists(atPath: prompts.path) { return prompts }
        return id == "claude" ? commands : prompts
    }
}
