...
/// Copies anything about to change into `~/Library/Application Support/Loadout/backups/<timestamp>/`.
///
...
public struct Backups: Sendable {
    ...
    public func snapshot(_ source: URL, stamp: Date = Date()) throws -> URL? {
        ...
        do {
            ...
            // Two writes inside the same second land in the same folder, and deleting what was
            // already there threw away the older copy — the one holding the state before any of
            ...
            while fm.fileExists(atPath: destination.path) {
                ...
                destination = destination.deletingLastPathComponent().appendingPathComponent(unique)
                attempt += 1
            }
