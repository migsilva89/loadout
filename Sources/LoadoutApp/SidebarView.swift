...
/// on its own full-width line, blue tokens naming whatever is currently narrowing the list,
...
struct SidebarView: View {
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    popoverRow(
                        title: "Global", subtitle: nil, checked: model.context == nil,
    ...
    /// Every narrowing gets a blue token the design's way: visible, named, one click to undo.
        ...
        ChipFlow(spacing: 5, lineSpacing: 5) {
            ForEach(activeTokens) { token in
                Button(action: token.clear) {
                    ...
                    .font(.system(size: 11))
                    .foregroundStyle(Color(red: 0.81, green: 0.89, blue: 1.0))
                    .padding(.leading, 8)
                    .padding(.trailing, 6)
                    .frame(height: 20)
                    .background(V2.accent.opacity(0.22), in: RoundedRectangle(cornerRadius: 5))
                    .overlay(RoundedRectangle(cornerRadius: 5).strokeBorder(V2.accent.opacity(0.45), lineWidth: 0.5))
...
/// a mini switch, and a two-line description. Selection paints the whole row the accent blue.
                    ...
                    Text(item.name)
                        ...
                        .foregroundStyle(
                            isSelected ? Color.white
                                : (item.enabled ? V2.text : V2.textDim)
                        )
                        .lineLimit(1)
                    if item.kind == .skill {
                        HStack(spacing: 3) {
...
/// 30×18 on every list row, 40×24 as the detail header's single big control. Green when on.
struct MiniSwitch: View {
    ...
    var body: some View {
        Button(action: action) {
            Capsule()
                .fill(on ? V2.green : Color.white.opacity(0.16))
