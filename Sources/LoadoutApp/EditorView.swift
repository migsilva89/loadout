...
struct MarkdownEditor: NSViewRepresentable {
    ...
    var original: String
    /// When the assistant has proposed changes, the document is shown with both sides of each of
    /// them opened up in place, and typing is off until they are decided — editing a buffer that
    /// holds lines which are not in the file would have no meaning.
    var review: ReviewLayout?
    /// Where each undecided change ended up on screen, so the Accept and Reject buttons can sit
    /// beside it. Reported on every layout and every scroll.
    var onReviewFrames: ([Int: CGRect]) -> Void = { _ in }
    var onEdit: () -> Void
    ...
    func makeNSView(context: Context) -> NSView {
        ...
        textView.insertionPointColor = NSColor(V2.caret)
        ...
        textView.string = review?.text ?? text
        textView.isEditable = review == nil
        ...
        context.coordinator.boundsObserver = NotificationCenter.default.addObserver(
            ...
            MainActor.assumeIsolated {
                ...
                // The buttons ride with the text they belong to, so scrolling has to move them.
                context.coordinator.reportReviewFrames()
            }
        }
    ...
    func updateNSView(_ scroll: NSView, context: Context) {
        ...
        // While changes are being reviewed the buffer is the review's, not the draft's.
        let wanted = review?.text ?? text
        textView.isEditable = review == nil
        // Only push text in when it truly changed elsewhere (Revert, a new selection, a decision
        // taken) — echoing every keystroke back in would fight the caret.
        if textView.string != wanted {
    ...
    final class Coordinator: NSObject, NSTextViewDelegate {
        ...
        func refresh(original: String) {
            ...
            if let review = parent.review {
                // The proposed lines are painted over the syntax colouring, which stays: a change
                // you can't read the markdown of is a change you can't judge.
                MarkdownHighlighter.paintReview(review, in: textView)
                gutter?.reviewKinds = review.kinds
        ...
        /// Measures where each undecided change sits in the visible text and hands the rectangles
        /// out, so the Accept and Reject buttons can be drawn against them.
        func reportReviewFrames() {
            guard let textView, let layout = textView.layoutManager, let container = textView.textContainer,
                  let review = parent.review, review.hasPending
            else {
                if !lastFrames.isEmpty {
                    lastFrames = [:]
                    let report = parent.onReviewFrames
                    DispatchQueue.main.async { report([:]) }
                }
                return
            }

            // Line index → character range, walked once rather than per block.
            let source = textView.string as NSString
            var lineStarts: [Int] = []
            var location = 0
            ...
            for (block, lines) in review.pendingRanges {
                guard lines.lowerBound < lineStarts.count else { continue }
                let start = lineStarts[lines.lowerBound]
                let end = lines.upperBound < lineStarts.count ? lineStarts[lines.upperBound] : source.length
                let characters = NSRange(location: start, length: max(0, end - start))
            ...
            lastFrames = frames
            // Never write SwiftUI state synchronously from make/update — this runs during both.
            let report = parent.onReviewFrames
            DispatchQueue.main.async { report(frames) }
        }
        ...
        /// Which line numbers differ from the file on disk — the gutter's accent bars. A real
...
/// frontmatter keys pink and values red, headings white on a dim hash, inline code in the
/// theme's code hue on a faint chip of it, fenced blocks in the command yellow, and the
/// frontmatter region lifted a step above the editor's ground.
///
/// The syntax hues are the same in every theme, and on purpose: they say what a run of
/// characters *is*, the way they do in Xcode, so a person who has learned that pink means a
/// frontmatter key does not have to learn it again per decor. What the theme does own here is
/// the ground, the caret and the code hue — the three the palette names.
enum MarkdownHighlighter {
    ...
    static var code: NSColor { NSColor(V2.code) }
    ...
    static var issueRed: NSColor { NSColor(V2.issue) }

    /// Main-actor: every `NSTextView` it reads is, and it is only ever called from the view.
    @MainActor
    static func apply(to textView: NSTextView) {
        ...
        while location < source.length {
            ...
            while true {
                ...
                storage.addAttribute(.backgroundColor, value: NSColor(V2.code.opacity(0.14)), range: span)
    ...
    /// Green behind lines the assistant wants to add, red behind the ones it wants to replace, and
    /// a quieter green behind the ones already accepted.
    ///
    /// Backgrounds rather than replacing the text colours: the markdown stays highlighted
    /// underneath, because a proposed line you can't read as markdown is one you can't judge.
    @MainActor
    static func paintReview(_ review: ReviewLayout, in textView: NSTextView) {
        guard let storage = textView.textStorage else { return }
        let source = textView.string as NSString
        storage.beginEditing()
        var location = 0
        var index = 0
        while location < source.length, index < review.kinds.count {
            let lineRange = source.lineRange(for: NSRange(location: location, length: 0))
            switch review.kinds[index] {
            case .context:
                break
            case .removed:
                storage.addAttribute(.backgroundColor, value: removedGround, range: lineRange)
                storage.addAttributes([
                    .strikethroughStyle: NSUnderlineStyle.single.rawValue,
                    .strikethroughColor: NSColor(white: 1, alpha: 0.35),
                    .foregroundColor: NSColor(white: 1, alpha: 0.45),
                ], range: NSRange(location: lineRange.location, length: max(0, lineRange.length - 1)))
            case .added:
                storage.addAttribute(.backgroundColor, value: addedGround, range: lineRange)
            case .accepted:
                storage.addAttribute(.backgroundColor, value: acceptedGround, range: lineRange)
            }
            index += 1
    ...
    static var addedGround: NSColor { NSColor(V2.ok.opacity(0.16)) }
    static var acceptedGround: NSColor { NSColor(V2.ok.opacity(0.07)) }
    static var removedGround: NSColor { NSColor(V2.issue.opacity(0.14)) }
...
/// Line numbers plus the two indicators the design asks the gutter to carry: a 2pt accent bar
...
final class LineGutter: NSView {
    ...
    /// While changes are being reviewed the line numbers stop meaning "line in the file" — the
    /// buffer holds both sides of every undecided change — so the gutter shows a − or a + instead of
    /// a number, and no bars.
    var reviewKinds: [ReviewLayout.Kind] = []
    ...
    override func draw(_ rect: NSRect) {
        ...
        while location < NSMaxRange(chars) {
            ...
            let kind = lineNumber - 1 < reviewKinds.count ? reviewKinds[lineNumber - 1] : nil
            var number = "\(lineNumber)" as NSString
            var ink = NSColor(white: 1, alpha: lineNumber == currentLine ? 0.7 : 0.25)
            switch kind {
            case .removed: number = "−"; ink = MarkdownHighlighter.issueRed
            case .added, .accepted: number = "+"; ink = NSColor(V2.ok)
